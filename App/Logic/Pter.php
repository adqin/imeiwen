...
class Pter {
    ...
    private function setCache() {
        ...
        // 查询作者与关键词关联的主题.
        $in = "('" . implode("','", $all) . "')";
        $where = "`post_keyword` in{$in} and `status` = '1' and `count` >= 3";
        ...
        $where = "`category` like '%$category%' and `post_status` like '%$post_status%' and `status` = 1 and `count` >= 3 and `post_keyword` = ''";
