...
class Updater {
    ...
    public static function updateTopicInfo($id = 0, $keyword = '', $type = '') {
        ...
        if ($type == 'keyword') {
            $keyword = ',' . $keyword . ',';
            $post_ids = \Db::instance()->getColumn("select `post_id` from `post` where `keywords` like '%$keyword%'");
        ...
        if ($type == 'author') {
            $author = $keyword;
            $post_ids = \Db::instance()->getColumn("select `post_id` from `post` where `author` = '$author'");
            $content = $post_ids ? implode(',', $post_ids) : '';
            $count = $post_ids ? count($post_ids) : 0;
            $param = [
                'content' => $content,
