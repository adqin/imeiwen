                    ...
                    <ul>
                        <?php foreach ($data_list as $data): ?>
                            <li><span class="post_list_title"><a href="/<?= $data['post_id'] ?>" target="_blank"><?= $data['title']; ?></a></span>  
                                ...
                                （<?= $data['word_count']; ?>字，<span style="color: red">浏览：<?= $data['page_view']; ?></span> &nbsp;&nbsp;
                                <?= date('Y-m-d H:i:s', $data['update_time']); ?>)  <a href="/admin/edit/<?= $data['post_id']; ?>">修改</a> <a href="/delete/<?= $data['post_id']; ?>" todel="<?= $data['post_id']; ?>">删除</a></li>
                            <?php endforeach; ?>
                    </ul>
        ...
        <script type="text/javascript">
            $(function () {
                $('a[todel]').click(function () {
                    ...
                    $.post('/admin/delete', {post_id: post_id}, function (re) {
                        re = $.parseJSON(re);
                        if (re.error == 1) {
                            alert(re.message);
                        }
